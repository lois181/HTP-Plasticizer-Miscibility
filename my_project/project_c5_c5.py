...
c5_c5_sim = MyProject.make_group(name="c5_c5_sim") 

def condition_c5_c5(job): # defining my condition for C5_C5 simulation
...
def grompp_npt(): 
    ...
    cmd = (
        "{gmx} grompp -c {gro_file} -f {npt_mdp} -n {index} -p {topol} -o topol-npt.tpr"
        .format(
            gmx=gmx_mpi,
...
def mdrun_npt():
    ''' helper function for generic mdrun '''
    cmd = (
        """{gmx} mdrun -s topol-npt.tpr -tableb table_a1.xvg
        mv confout.gro confoutNPT.gro"""
        .format(
            gmx=gmx_mpi)
        )
    return cmd
...
@c5_c5_sim       
@MyProject.pre(condition_c5_c5)
@MyProject.post.isfile("topol-npt.tpr")
@MyProject.operation(with_job=True, cmd=True)
...
#@c5_c5_sim
#@MyProject.pre(condition_c5_c5)
#@MyProject.pre.isfile("confout.gro")
#@MyProject.post.isfile("confoutNPT.gro")
#@MyProject.operation(with_job=True, cmd=True)
...
@MyProject.post.isfile("poly1.txt")
@MyProject.operation(with_job=True, cmd=True)
def get_ent_files(job):
    ...
    echo "$i" | gmx_mpi polystat -f traj_comp.xtc -n index_ent.ndx -s topol.tpr -o poly"$i".xvg
...
def perform_analysis(job):
    with job:
        ...
        no1 = '@'
        no2 = '&'
        start = 200 # time to start at
        space = 100 # time between tests
...
@c5_c5_sim
@MyProject.pre(condition_c5_c5)
@MyProject.pre.isfile("poly1.xvg")
...
@c5_c5_sim
@MyProject.pre(condition_c5_c5)
@MyProject.pre.isfile("#poly1.xvg.1#")
@MyProject.operation(with_job=True, cmd=True)
def clean4(job):
    """ removing the poly files because there are too many """
    with job:
